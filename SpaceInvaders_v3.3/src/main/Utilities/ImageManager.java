...
public class ImageManager {
    ...
    private static Image[] imagesExplosion;

    public static Image[] getExplosion() {
        if (imagesExplosion == null){
            imagesExplosion = new Image[6];
            for (int i = 0; i<6; i++ ) {
                imagesExplosion[i] = Toolkit.getDefaultToolkit().createImage(C.PATH_EXPLOSION_F[i]);
            }
        }
